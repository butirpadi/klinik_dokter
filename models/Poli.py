...
class Poli(models.Model):
    ...
    def get_line_graph_datas(self):
        ...
        def build_graph_data(date, amount):
            ...
            name = format_date(date, 'd LLLL Y')
            short_name = format_date(date, 'd MMM')
            return {'x':short_name,'y': amount, 'name':name}
        ...
        # SaleOrderStmnt = self.env['sale.order']
        # last_stmt = SaleOrderStmnt.search([('poli_id', '=', self.id), ('date_order', '=', today.strftime(DF))], order='date_order desc, id desc')

        # last_balance = last_stmt and len(last_stmt) or 0
        ...
        date = today
        amount = 0
        # query = """SELECT l.date_order as date, count(l.id) as amount
        #                 from sale_order l
        #                 WHERE is_appointment_order = true 
        #                   AND invoice_status = 'invoiced'
        #                   AND l.poli_id = %s
        #                   AND l.date_order > %s
        #                   AND l.date_order <= %s
        #                 GROUP BY l.date_order
        #                 ORDER BY l.date_order desc
        #                 """
        
        # query = """select date_order::timestamp::date as date, count(sale_order.id) as amount
        #                 from sale_order
        #                 where 
        #                 invoice_status = 'invoiced'
        #                 AND poli_id = %s
        #                 AND date_order::timestamp::date > %s
        #                 AND date_order::timestamp::date <= %s
        #                 group by date
        #                 order by date desc
        #                 """
        query = """select (date_order at time zone 'utc' at time zone 'asia/jakarta')::timestamp::date as date, count(id) as amount 
                        from sale_order
                        where 
                        invoice_status = 'invoiced'
                        AND poli_id = %s
                        AND (date_order at time zone 'utc' at time zone 'asia/jakarta')::timestamp::date > %s
                        AND (date_order at time zone 'utc' at time zone 'asia/jakarta')::timestamp::date <= %s
                        group by date
                        order by date desc
                        """
        ...
        if len(query_result) > 0:
            for val in query_result:
                data[:0] = [build_graph_data(last_month, val['amount'])]
        else:
                data[:0] = [build_graph_data(last_month, 0)]
